...
# from config_cResNet import opt
...
import torch as t
import models
from models import ResNet50, cResNet51
...
def train(**kwargs):
    ...
    optimizer = t.optim.SGD(c_resnet_51.parameters(), lr=lr, momentum=opt.momentum, weight_decay=opt.weight_decay)
    start_epoch = 0

    if opt.resume:
        start_epoch = c_resnet_51.module.load(None if opt.finetune else optimizer, opt.resume, opt.finetune)
    ...
    for epoch in range(start_epoch+1, opt.max_epoch+1):
        ...
        val_class_loss, val_top5_acc, val_top1_acc = val(compression_model, c_resnet_51, val_dataloader, class_loss, None, ps)
...
def val(compression_model, c_resnet_51, dataloader, class_loss, rate_loss=None, ps=None):
    if ps:
        ps.log ('validating ... ')
    else:
        print ('run val ... ')
