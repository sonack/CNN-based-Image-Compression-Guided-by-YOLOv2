...
from unet_train_fr_gr_config import opt
import os
import numpy as np
import math
import torch as t
...
def train(**kwargs):
    ...
    ps = PlotSaver(opt.exp_id[4:]+"_"+time.strftime("%m_%d__%H:%M:%S")+".log.txt")

    # step1: Model

    # teacher Unet model

    t_model = getattr(models, opt.t_model)(use_unet = True, use_imp = False, model_name = "UNet_teacher_network_loaded")
    model = getattr(models, opt.model)(use_unet = False, use_imp = opt.use_imp, model_name="TPSE_p{ps}_imp_r={r}_gama={w}".format(
                                                                ps=opt.patch_size,
                                                                r=opt.rate_loss_threshold, 
                                                                w=opt.rate_loss_weight)   # w is the unet_loss weight
                                                                if opt.use_imp else "UNet_Fr_Gr_p{ps}_w{w}".format(ps=opt.patch_size, w=opt.unet_loss_weight))
    ...
    if opt.use_gpu:
        t_model = multiple_gpu_process(t_model)
        model = multiple_gpu_process(model)
    



     # freeze the encoder network, freeze mode
    ...
    optimizer = t.optim.Adam(filter(lambda p: p.requires_grad, model.parameters()), lr=lr, betas=(0.9, 0.999))
    ...
    if opt.t_ckpt:
        if use_data_parallel:
            t_model.module.load(None, opt.t_ckpt, True)
        else:
            t_model.load(None, opt.t_ckpt, True)
    ...
    unet_loss_meter = AverageValueMeter()
    ...
    for epoch in range(start_epoch+1, opt.max_epoch+1):
        ...
        if (epoch == start_epoch + 1) and opt.init_val:
            ...
            if opt.use_imp:
                mse_val_loss, rate_val_loss, total_val_loss, rate_val_display = val(model, t_model, val_dataloader, mse_loss, rate_loss, ps, -1, True)
            else:
                unet_val_loss, mse_val_loss = val(model, t_model, val_dataloader, mse_loss, None, ps, -1, True)
            ...
            if opt.use_imp:
                ...
                ps.log('Init Val @ Epoch:{epoch}, lr:{lr}, val_mse_loss:{val_mse_loss}, val_unet_loss:{val_unet_loss} '.format(
                    epoch = epoch,
                    lr = lr,
                    val_mse_loss = mse_val_loss,
                    val_unet_loss = unet_val_loss
                ))
        ...
        for idx, (data, _) in enumerate(train_dataloader):
            ...
            ipt = Variable(data)
            t_ipt = Variable(data, volatile=True)

            if opt.use_gpu:
                ipt = ipt.cuda(async = True)
                t_ipt = t_ipt.cuda(async = True)


            optimizer.zero_grad()

            reconstructed, d0, d1, d2 = model(ipt, expose_skip = True)
            t_reconstructed, t_d0, t_d1, t_d2 = t_model(t_ipt, expose_skip = True)
            ...
            unet_loss = (mse_loss(d0, t_d0) + mse_loss(d1, t_d1) + mse_loss(d2, t_d2)) * opt.unet_loss_weight
            
            if opt.use_imp:
                rate_loss_display = imp_mask_sigmoid
                rate_loss_ =  rate_loss(rate_loss_display)
    
                total_loss = caffe_loss + rate_loss_ + unet_loss
            else:
                total_loss = caffe_loss + unet_loss
            ...
            unet_loss_meter.add(unet_loss.data[0])
            ...
            if idx % opt.print_freq == opt.print_freq - 1:
                ...
                if not opt.use_imp:
                    ps.log('Epoch %d/%d, Iter %d/%d, loss = %.2f, mse_loss = %.2f, unet_loss = %.2f, lr = %.8f' % (epoch, opt.max_epoch, idx, len(train_dataloader), total_loss_meter.value()[0], mse_loss_meter.value()[0], unet_loss_meter.value()[0], lr))
                else:
        ...
        # val 
        if opt.use_imp:
            mse_val_loss, rate_val_loss, total_val_loss, rate_val_display = val(model, t_model, val_dataloader, mse_loss, rate_loss, ps, epoch, epoch % opt.show_inf_imgs_T == 0)
        else:
            unet_val_loss, mse_val_loss = val(model, t_model, val_dataloader, mse_loss, None, ps, epoch, epoch % opt.show_inf_imgs_T == 0)
...
val_mse_loss: {val_mse_loss}, val_rate_loss: {val_rate_loss}, val_total_loss: {val_total_loss}, val_rate_display: {val_rate_display} '.format(
            ...
            ps.log('Epoch:{epoch}, lr:{lr}, train_mse_loss:{train_mse_loss}, train_unet_loss:{train_unet_loss}, val_mse_loss:{val_mse_loss}, val_unet_loss:{val_unet_loss}'.format(
                epoch = epoch,
                lr = lr,
                train_mse_loss = mse_loss_meter.value()[0],
                train_unet_loss = unet_loss_meter.value()[0],
                val_mse_loss = mse_val_loss,
                val_unet_loss = unet_val_loss
            ))
        ...
        if opt.use_grad_mode and epoch % opt.alpha_decay_epochs == 0:
            opt.unet_loss_weight *= opt.alpha_decay_factor
            ps.log("alpha decay to %f" % opt.unet_loss_weight)
...
def val(model, t_model, dataloader, mse_loss, rate_loss, ps, epoch, show_inf_imgs=True):
    ...
    for idx, (data, _) in enumerate(dataloader):
        ...
        reconstructed, d0, d1, d2 = model(val_input, expose_skip = True)
        t_reconstructed, t_d0, t_d1, t_d2 = t_model(val_input, expose_skip = True)
        ...
        if opt.use_imp and rate_loss:
            ...
            total_loss = batch_caffe_loss + unet_loss
    ...
    if opt.use_imp:
        ...
        return unet_loss_meter.value()[0], mse_loss_meter.value()[0]
